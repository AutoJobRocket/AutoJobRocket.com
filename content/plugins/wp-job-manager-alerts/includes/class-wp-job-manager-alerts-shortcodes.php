...
class WP_Job_Manager_Alerts_Shortcodes {
				...
				switch ( $this->action ) {
					...
					case 'edit' :
						if ( isset( $_POST['submit-job-alert'] ) ) {
							...
							if ( empty( $alert_name ) )
								throw new Exception( __( 'Please name your alert', 'job_manager_alerts' ) );
							...
							if ( $this->action == 'add_alert' ) {
								...
								if ( $alert->post_author != get_current_user_id() )
									throw new Exception( __( 'Invalid Alert', 'job_manager_alerts' ) );
							...
							if ( taxonomy_exists( 'job_listing_category' ) ) {
								$alert_cats = isset( $_POST['alert_cats'] ) ? array_map( 'sanitize_title', $_POST['alert_cats'] ) : array();

								if ( $alert_cats )
									wp_set_object_terms( $alert_id, $alert_cats, 'job_listing_category' );
							}

							$alert_job_type = isset( $_POST['alert_job_type'] ) ? array_map( 'sanitize_title', $_POST['alert_job_type'] ) : array();
							...
							if ( $this->action == 'add_alert' ) {
								...
								// Create cron
								wp_schedule_single_event( $next, 'job-manager-alert', array( $alert_id ) );
							}
					...
					case 'toggle_status' :
						...
						// Message
						$this->alert_message = '<div class="job-manager-message">' . sprintf( __( '%s has been %s', 'job_manager_alerts' ), $alert->post_title, $alert->post_status == 'draft' ? __( 'Enabled', 'job_manager_alerts' ) : __( 'Disabled', 'job_manager_alerts' ) ) . '</div>';
					break;
					case 'delete' :
						$alert_id = absint( $_REQUEST['alert_id'] );
						$alert    = get_post( $alert_id );
						...
						// Message
						$this->alert_message = '<div class="job-manager-message">' . sprintf( __( '%s has been deleted', 'job_manager_alerts' ), $alert->post_title ) . '</div>';
					break;
				}
	...
	public function job_alerts( $atts ) {
		...
		if ( ! is_user_logged_in() ) {
			_e( 'You need to be signed in to manage your alerts.', 'job_manager_alerts' );
			return;
		}
		...
		if ( ! empty( $_GET['updated'] ) )
			echo '<div class="job-manager-message">' . __( 'Your alerts have been updated', 'job_manager_alerts' ) . '</div>';
		else
		...
		get_job_manager_template( 'my-alerts.php', array( 'alerts' => $alerts, 'user' => $user ), 'job_manager_alerts', JOB_MANAGER_ALERTS_PLUGIN_DIR . '/templates/' );
	...
	public function add_alert() {
		get_job_manager_template( 'alert-form.php', array(
			...
			'alert_cats'      => isset( $_REQUEST['alert_cats'] ) ? array_map( 'sanitize_title', $_REQUEST['alert_cats'] ) : array(),
			'alert_job_type'  => isset( $_REQUEST['alert_job_type'] ) ? array_map( 'sanitize_title', $_REQUEST['alert_job_type'] ) : array()
		), 'job_manager_alerts', JOB_MANAGER_ALERTS_PLUGIN_DIR . '/templates/' );
	}
	...
	public function edit_alert( $alert_id ) {
		...
		get_job_manager_template( 'alert-form.php', array(
			...
			'alert_cats'      => isset( $_POST['alert_cats'] ) ? array_map( 'sanitize_title', $_POST['alert_cats'] ) : wp_get_post_terms( $alert_id, 'job_listing_category', array( 'fields' => 'slugs' ) ),
	...
	public function view_results( $alert_id ) {
		...
		echo wpautop( sprintf( __( 'Jobs matching your "%s" alert:', 'job_manager_alerts' ), $alert->post_title ) );
		...
		<?php else :
			...
			echo wpautop( __( 'No jobs found', 'job_manager_alerts' ) );
