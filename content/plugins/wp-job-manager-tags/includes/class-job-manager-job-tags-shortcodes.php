...
class WP_Job_Manager_Job_Tags_Shortcodes {
	...
	public function show_tag_filter( $shortcode_atts ) {
		...
		if ( $shortcode_atts['show_tags'] === false || (string) $shortcode_atts['show_tags'] == 'false' )
			return;
		...
		$html = '<div class="filter_wide filter_by_tag">' . __( 'Filter by tag:', 'job_manager_tags' ) . ' ' . wp_tag_cloud( apply_filters( 'job_filter_tag_cloud', $atts ) ) . '</div>';
	...
	public function apply_tag_filter( $args ) {
		...
		if ( isset( $params['job_tag'] ) ) {
			$args['tax_query'][] = array(
				'taxonomy' => 'job_listing_tag',
				'field'    => 'name',
				'terms'    => array_filter( $params['job_tag'] ),
				'operator' => "IN"
			);
	...
	public function apply_tag_filter_text( $text ) {
		...
		$text .= ' ' . __( 'tagged', 'job_manager_tags' ) . ' ' . implode( ', ', array_filter( $params['job_tag'] ) );
	...
	public function jobs_by_tag( $atts ) {
		...
		<?php else :
			...
			echo '<p>' . sprintf( __( 'No jobs found tagged with %s.', 'job_manager_tags' ), implode( ', ', $tags ) ) . '</p>';
	...
	public function tag_cloud_text_callback( $count ) {
		return sprintf( _n( '%s job', '%s jobs', $count, 'job_manager_tags' ), number_format_i18n( $count ) );
	}
}
